	...
	<div class="navbar">
		...
		<el-dropdown class="dropdown-box" @command="handleCommand" trigger="click">
			<div class="el-dropdown-link">
				<el-image v-if="guanliyuan" :src="avatar?this.$base.url + avatar : require('@/assets/img/avator.png')" fit="cover"></el-image>
			...
			<el-dropdown-menu class="top-el-dropdown-menu" slot="dropdown">
				...
				<el-dropdown-item v-if="this.$storage.get('role')!='管理员'" class="item3" :command="'front'" style="display:none;">
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 		}
			// 		if(sessionTable == 'qiuyuan') {
			// 			this.$storage.set('headportrait',data.data.touxiang)
			// 		}
			// 		if(sessionTable == 'jiaolian') {
			// 			this.$storage.set('headportrait',data.data.touxiang)
			// 		}
			// 		if(sessionTable == 'caipan') {
			// 			this.$storage.set('headportrait',data.data.touxiang)
			// 		}
